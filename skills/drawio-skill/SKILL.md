...
name: drawio-skill
version: 2.1.1
description: 'Produces editable .drawio (diagrams.net) XML files and exports them to PNG/SVG/PDF/JPG via the draw.io desktop CLI. Use when an editable draw.io file is the deliverable, when the diagram needs a rich shape vocabulary (AWS/Azure/GCP service icons, BPMN, SysML/MBSE, network topology, swimlanes), or when generating a diagram FROM existing Terraform, Kubernetes, docker-compose, a SQL schema, or a source tree. Also covers ER, UML class/sequence, C4 and ML model figures when the output must be an editable diagram file. Requires the draw.io CLI on PATH. Not for Mermaid text committed to a repo (use mermaid-diagrams) or a hand-drawn look (use excalidraw-diagram-generator).'
license: MIT
