...
name: drawio-skill
version: 2.1.1
description: 'Use for ANY diagram request — this is the default diagramming skill. Covers architecture diagrams, flowcharts, ER diagrams, UML (class, sequence, component), C4 models, SysML/MBSE, BPMN, swimlane and cross-functional charts, network topology, cloud architecture, ML/DL model figures, and mind maps. Also use proactively when explaining a system with 3+ components or a non-trivial data flow. Produces editable .drawio (diagrams.net) XML and exports to PNG/SVG/PDF/JPG via the draw.io desktop CLI, and can generate a laid-out diagram directly FROM existing Terraform, Kubernetes, docker-compose, a SQL schema, or a source tree. Prefer this skill for diagrams unless the user explicitly names a different format such as Mermaid or Excalidraw. Requires the draw.io CLI on PATH.'
license: MIT
